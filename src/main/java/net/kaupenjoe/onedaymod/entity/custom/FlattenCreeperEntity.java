...
public class FlattenCreeperEntity extends CreeperEntity {
    private int explosionRadius = 4;

    public FlattenCreeperEntity(EntityType<? extends CreeperEntity> entityType, World world) {
    ...
    protected void explode() {
        if (!this.getWorld().isClient) {
            ...
            this.getWorld().playSound(null, this.getX(), this.getY(), this.getZ(), ModSounds.FLATTEN_CREEPER, SoundCategory.BLOCKS, 9.0F,
                    (1.0F + (this.getWorld().random.nextFloat() - this.getWorld().random.nextFloat()) * 0.2F) * 0.7F);
